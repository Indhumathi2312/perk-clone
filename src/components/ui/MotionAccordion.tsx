...
function AccordionItem({
  ...
  return (
    <motion.div
      layout
      className={cn(
        "overflow-hidden rounded-[30px] bg-[#18191c] text-white border border-white/10",
      )}
    ...
    >
      ...
      >
        <span className="text-[clamp(1.05rem,1.6vw,1.25rem)] font-medium tracking-tight leading-snug">
          {item.question}
        </span>
        ...
        <motion.span
          ...
          className="inline-flex size-12 shrink-0 items-center justify-center text-white"
        >
          {isOpen ? (
      ...
      >
        <motion.div
          ...
          className="px-7 pb-7"
        >
          <p className="text-base sm:text-lg leading-8 font-normal tracking-normal text-white/70">
            {item.answer}
          </p>
        </motion.div>
      </motion.div>
...
export function MotionAccordion({
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className="flex flex-col rounded-[34px] p-1 sm:p-3" style={{ gap }}>
        {items.map((item, i) => (
          <AccordionItem
            key={i}
            item={item}
